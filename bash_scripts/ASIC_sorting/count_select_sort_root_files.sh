...
# Filter only files ending with elect.root or holes.root
FILES=($(find "$DIR" -maxdepth 1 -type f -name "*elect.root" -o -name "*holes.root"))
...
select_most_recent() {
    ...
    for file in "${files[@]}"; do
        ...
        # Extract ASIC number (HW_X) and polarity (holes/elect.root)
...
sort_files() {
    ...
    for file in "${files[@]}"; do
        if [[ "$file" == *holes.root ]]; then
            holes_sorted+=("$file")
        elif [[ "$file" == *elect.root ]]; then
            elect_sorted+=("$file")
        fi
    done
